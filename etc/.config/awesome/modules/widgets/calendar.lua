...
-- ░▀░▀░▀▀▀░▀▀░░▀▀▀░▀▀▀░░▀░
--
-- Modified wibox.widget.calendar + from https://github.com/streetturtle/awesome-wm-widgets/tree/master/calendar-widget
---------------------------------------------------------------------------------------------------
...
local function worker(user_args)
    local calendar_themes = {
        default = {
            bg = beautiful.palette.bg_lighter2,
            fg = beautiful.palette.fg_darker2,

            focus_date_bg = beautiful.palette.primary,
            focus_date_fg = beautiful.palette.fg,

            weekend_day_bg = beautiful.palette.bg_lighter5,
            weekday_fg = beautiful.palette.fg_lighter1,

            header_fg = beautiful.palette.fg,
            border = beautiful.palette.bg_lighter5,
        },
    }
        ...
        if popup.visible then
            ...
            elseif placement == "top_right" then
                awful.placement.top_right(
                    popup,
                    { margins = { top = beautiful.wibar_height + 10, right = 10 }, parent = awful.screen.focused() }
                )
            ...
            elseif placement == "bottom_right" then
                awful.placement.bottom_right(
                    popup,
                    { margins = { bottom = 30, right = 10 }, parent = awful.screen.focused() }
                )
            elseif placement == "bottom_left" then
                awful.placement.bottom_left(
                    popup,
                    { margins = { bottom = 30, left = 10 }, parent = awful.screen.focused() }
                )
            else
                awful.placement.top(popup, { margins = { top = 30 }, parent = awful.screen.focused() })
            end
